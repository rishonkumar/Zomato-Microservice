...
interface props {
    fetchMyRes: () => Promise<void>
}

const AddRestaurant = ({ fetchMyRes }: props) => {
    ...
    const { loadingLocation, location } = userAppData()
    ...
    const handleSubmit = async () => {
        ...
        formData.append("formattedAddress", location.formattedAdress)
        ...
        try {
            ...
            toast.success("Restaurant added sucessfullly")
            fetchMyRes()
        } catch (err: any) {
            ...
            <div className="mx-auto max-w-lg rounded-xl bg-white p-6 shadow-sm space-y-5">
                ...
                <div className="flex items-start gap-3 rounded-lg border p-4">
                    ...
                    <div className="text-sm">
                        {loadingLocation ? "Fetching your location " : location?.formattedAdress || "Location not available"}
                    </div>
                </div>
                <button className="w-full rounded-lg py-3 text-sml font-semibold text-white bg-[#e23744]" disabled={submitting} onClick={handleSubmit}>
