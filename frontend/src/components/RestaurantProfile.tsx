...
const RestaurantProfile = ({ restaurant, isSeller, onUpdate }: props) => {
    ...
    const toggleOpenStatus = async () => {
        try {
            const { data } = await axios.put(`${restaurantService}/restaurant/status`, {
                status: !isOpen
            }, {
                headers: {
            ...
            <div className='p-5 spae-y-4'>
                {isSeller && (<div className='flex items-center justify-between'>
                    <div>
                        ...
                        <div className='mt-1 flex items-center gap-2 text-sm text-gray-500'>
                            ...
                            {restaurant.autoLocation.formattedAddress || "Location unavailabl"}
                        </div>
                    </div>
...

                {
                    editMode ?
                        ...
                        :
                        (<p className='text-sm text-gray-600'>{restaurant.description || "No descrption added"}</p>
                        )
                }
                <div className='flex items-center justify-between pt-3 border-t'>
                    ...
                    <div className='flex gap-3'>
                        ...
                        ><BiSave size={16} />Save</button>)}
                        {
                            isSeller && (<button onClick={toggleOpenStatus} className={`rounded-lg px-4 py-1.5 text-sm font-medium text-white ${isOpen ? "bg-red-600 hover:bg-red-700" : "bg-green-600 hover:bg-green-700"}`}>{isOpen ? "Closed res" : "Open res"}</button>
                            )}
                    </div>
                </div>
